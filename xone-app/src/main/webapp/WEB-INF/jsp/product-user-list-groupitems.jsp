...
<li class="productdatecreateditem" timestamp="<fmt:formatDate value="${item.dateCreated}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/>">
	...
	<a href="${pageContext.request.contextPath}/product/groupitemForUser.html?productGroup.id=${item.id}" style="padding-right:10px;padding-top:2px;padding-bottom:3px;padding-left:85px;">
		...
		<div style="margin-top:10px;text-overflow:ellipsis;">${p.productName}</div>
	    <div style="margin-top:8px;">
		    ...
		    ,类型:${p.productTypeName},预定数量:${item.groupNum},产品数量:${p.productNum}</p>
		    <p>产品编号:${p.id},<c:if test="${not empty item.remark}">审核备注:${item.remark},</c:if>产品描述:${p.productDesc}</p>
	    </div>
	</a>
</li>
</c:forEach>