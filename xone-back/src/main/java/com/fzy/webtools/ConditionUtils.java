...
public class ConditionUtils {
    ...
    public static void fillConditionsFromRequest(List queryConditions,
        ...
        try {
            ...
            for (int i = 0; i < conditions.length(); i++) {
                ...
                String paramName = c.getString("field");
                String[] paramValues = request.getParameterValues(paramName);
                if (paramValues != null) {
                    for (int j = 0; j < paramValues.length; j++) {
                        ConditionItem ci = new ConditionItem();
                        ci.setField(paramName);
                        queryConditions.add(ci);
                        queryParamValueList.add(paramValues[j]);
                    }
                }
            }
