...
<form id="myqueryform${myidentify}" action="${pageContext.request.contextPath}/roles/rolesListAjax.html" method="get">
...
<div>
	<table class="table table-bordered myroleslisttable">
		...
		<tr>
			...
			<td>
				<a href="#" attr-id="${item.id}" attr-name="${item.rolesname}" class="btn btn-mini rolesinfoselect">选择</a>
			</td>
		</tr>
		</c:forEach>
		</tbody>
...
<x:page href="${pageContext.request.contextPath}/roles/rolesListAjax.html" pagination="${pagination}"/>
...
$(document).ready(function() {
	...
	$('div.rolesbacklist${myidentify} .pagination a').on('click', function(e) {
		e.preventDefault();
		$.ajax({
			type: 'get',
			url: $(this).attr('href'),
